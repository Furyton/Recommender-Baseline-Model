...
def get_train_dataset(train_data, item_num, user_num, data_len, negative_num=2):
...
class BPR(nn.Module):
    ...
    def forward(self, user, item_i, item_j):
        ...
        return prediction_i, prediction_j, lamb * ((user * user).sum(dim=-1) + (item_i * item_i).sum(dim=-1) + (item_j * item_j).sum(dim=-1))
...
def train(train_data, item_num, user_num, data_len, answer_list, candidates_list):
    print("== training ==")
    ...
    model = BPR(user_num, item_num, 100)

    # model = torch.load('model_100dim.pth')
    ...
    for epoch in range(epochs):
        ...
        if epoch % 10 == 0:
            ...
            if HIT[10] > last_HIT:
                last_HIT = HIT[10]
                torch.save(model, 'model_100dim.pth')

    torch.save(model, 'model_100dim_final.pth')