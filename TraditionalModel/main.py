...
def get_data(dataset_name):
    x = {}
    with open(path.normpath(os.path.join('data', dataset_name)), 'r') as dataset_file:
        contents = dataset_file.readlines()
...
if __name__ == '__main__':
    ...
    user_num = len(data)
    #
    item_num = get_item_num(data)
    #
    print("== splitting dataset ==")
    #
    train, test = split_dataset(data)
    #
    popularity = get_popularity(train, item_num)
    #
    ...
    train = [[x - 1 for x in l] for l in train]
    ...
    #                                        epoch_num=args.epoch, lr=args.lr, dim=args.hidden_units,
